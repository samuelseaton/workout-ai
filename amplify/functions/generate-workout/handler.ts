...
function buildPrompt(inputs: WorkoutInputs): string {
...
- Tempo format is 4 phases: eccentric-bottom pause-concentric-top pause (e.g. "3-1-1-0" = 3s lower, 1s pause at bottom, 1s lift, 0s pause at top; use "X" for explosive concentric)
...
- For ${inputs.intensity} trainees: ${
    inputs.intensity === 'Beginner'
...
export const handler = async (event: {
  arguments: { inputs: string };
}) => {
  const inputs: WorkoutInputs = JSON.parse(event.arguments.inputs);
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are an expert personal trainer. Always respond with valid JSON only — no markdown fences, no preamble, just the raw JSON object.',
      },
      {
        role: 'user',
